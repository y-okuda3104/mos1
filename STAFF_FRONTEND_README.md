...
```
注文ステータス周期：
受け取り → 調理中 → 配膳完了 → 会計待ち → 退店

スタッフの操作：
- 各注文の「ステータス遷移ボタン」で段階的に進行
- 複数注文が同じ座席にある場合、個別に管理
- ダッシュボードで全体進捗をモニタリング
```
...
const StaffState = {
  ...
  // UI状態
  selectedSeatId: null,    // 詳細表示中の座席
  lastPolledAt: null,      // 最後のポーリング時刻
  isPolling: false,        // ポーリング中フラグ

  // 更新メソッド
  updateAllOrders: async function() {},
  updateOrderStatus: async function(orderId, newStatus) {},
  respondToAlert: async function(alertId) {},
  getOrdersBySeat: function(seatId) {}
};
```
